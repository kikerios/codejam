/* 
 * https://github.com/buunguyen/mongoose-deep-populate/blob/master/lib/plugin.js
 */
module.exports = function (mongoose) {
    ...
    function createParams(model, docs, paths, options, lean) {
        ...
        if (rewrite) {
            ...
            if (populate) {
                Object.keys(populate).forEach(function (path) {
                    ...
                    if (rewrittenPath)
                        populate[rewrittenPath] = populate[path]
                })
            }
        }
        ...
        paths.forEach(function (path) {
            ...
            if (noOfSubpaths > maxNoOfSubpaths)
                maxNoOfSubpaths = noOfSubpaths
        })

        var db = model.db
        if (!db) {
            var doc = isArray(docs) ? docs[0] : docs
            if (doc && doc.ownerDocument)
                db = doc.ownerDocument().constructor.db
            else
                throw new Error('Cannot retrieve database instance')
        }
